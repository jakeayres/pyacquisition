...
class Mercury_IPS(Instrument):
    ...
    def _parse_status_string(self, string: str, index: int):
        ...
        if not isinstance(string, str):
            raise TypeError(
                f"Expected to receive a string, got {type(string).__name__}"
            )

        elif len(string) not in [12, 15]:
            raise ValueError(
                f"Expected status string of length 12 or 15, got {string} (len {len(string)})"
            )

        elif string[0] != "X":
            raise ValueError(
                f'"X" not found at string[0]. Expected string of form XmnAnCnHnMmnPmn, got {string}'
            )

        elif string[3] != "A":
            raise ValueError(
                f'"A" not found at string[3]. Expected string of form XmnAnCnHnMmnPmn, got {string}'
            )
